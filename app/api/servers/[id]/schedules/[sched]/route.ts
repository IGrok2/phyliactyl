import { clientHandler } from "@/lib/pterodactyl/bff"
import { updateSchedule, deleteSchedule } from "@/lib/pterodactyl/api"
import { mapSchedule } from "@/lib/pterodactyl/map"

export async function PATCH(
  req: Request,
  { params }: { params: Promise<{ id: string; sched: string }> },
) {
  const { id, sched } = await params
  const b = (await req.json().catch(() => ({}))) as Record<string, unknown>
  return clientHandler(async () => {
    const updated = await updateSchedule(id, sched, {
      name: String(b.name ?? "Schedule"),
      minute: String(b.minute ?? "*"),
      hour: String(b.hour ?? "*"),
      day_of_month: String(b.day_of_month ?? "*"),
      month: String(b.month ?? "*"),
      day_of_week: String(b.day_of_week ?? "*"),
      is_active: b.is_active === undefined ? true : Boolean(b.is_active),
    })
    return mapSchedule(updated.attributes)
  })
}

export async function DELETE(
  _req: Request,
  { params }: { params: Promise<{ id: string; sched: string }> },
) {
  const { id, sched } = await params
  return clientHandler(async () => {
    await deleteSchedule(id, sched)
    return { ok: true }
  })
}
